...
local function generate_pink_noise(sample_rate, duration)
	local samples = sample_rate * duration
	local data = ffi.new("int16_t[?]", samples)

	local b0, b1, b2 = 0, 0, 0 -- Filter state variables
	local max_amplitude = 32767
	local min_amplitude = -32768

	for i = 0, samples - 1 do
		-- Generate white noise sample
		local white = math.random(-1000, 1000)

		-- Apply filtering (Paul Kellet's approximation)
		b0 = 0.99886 * b0 + white * 0.0555179
		b1 = 0.99332 * b1 + white * 0.0750759
		b2 = 0.96900 * b2 + white * 0.1538520

		local pink = b0 + b1 + b2 + white * 0.3104856

		-- Clamp to avoid overflow
		if pink > max_amplitude then
			pink = max_amplitude
		elseif pink < min_amplitude then
			pink = min_amplitude
		end
...
function M.play()
	...
	local noise_data, size = generate_pink_noise(44100, 5)
